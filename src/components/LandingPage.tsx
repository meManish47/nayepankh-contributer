...
export default function LandingPage({ onStart }: { onStart: () => void }) {
  return (
    <div className="flex flex-col min-h-screen bg-[#1c1917] text-white">
      {/* Header */}
      <header className="px-6 py-4 flex items-center justify-between border-b border-white/10 bg-[#1c1917]/80 backdrop-blur-md sticky top-0 z-10">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-md bg-white flex items-center justify-center">
            <Sparkles className="w-5 h-5 text-[#1c1917]" />
          </div>
          <span className="font-bold text-xl tracking-tight text-white">NayePankh Foundation</span>
        </div>
        <Button variant="outline" className="hidden sm:flex bg-transparent text-white border-white/20 hover:bg-white hover:text-[#1c1917]" onClick={onStart}>
          Join the Team
        </Button>
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 pt-20 pb-16 text-center">
        <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
          <div className="inline-flex items-center rounded-full border px-2.5 py-0.5 text-sm font-semibold border-white/20 bg-white/5 text-white">
          ...
          <p className="text-xl text-stone-300 leading-relaxed max-w-2xl mx-auto">
            "Join our team and make a difference in the lives of those in need." An intelligent application experience that helps you discover how you can contribute to meaningful social impact initiatives.
          </p>
          
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
            <Button size="lg" className="w-full sm:w-auto text-base h-12 px-8 bg-white text-[#1c1917] hover:bg-stone-200" onClick={onStart}>
              JOIN US
              <ArrowRight className="ml-2 w-5 h-5" />
            </Button>
            <Button size="lg" variant="outline" className="w-full sm:w-auto text-base h-12 px-8 bg-transparent text-white border-white/20 hover:bg-white/10 hover:text-white">
              Learn More
            </Button>
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="border-t border-white/10 py-8 text-center text-stone-500 text-sm">
...
function FeatureCard({ icon, step, title, description }: { icon: React.ReactNode, step: string, title: string, description: string }) {
  return (
    <Card className="border-none shadow-md bg-white/5 backdrop-blur-sm hover:bg-white/10 transition-all duration-300 group">
      <CardContent className="pt-6">
        ...
        <div className="text-sm font-semibold text-stone-300 mb-2">{step}</div>
